...
xcoords=[0.27:0.01:0.29];
for p=1:3
    if p==2
        hann(p)=annotation('textarrow',[xcoords(p) xcoords(p)],[0.94 0.88],'String','Photons');
        set(hann(p),'Color','r');
    else
        hann(p)=annotation('textarrow',[xcoords(p) xcoords(p)],[0.94 0.88],'String','');
        set(hann(p),'Color','r');
    end
end

hann=annotation('textarrow',[0.28 0.48],[0.874 0.874],'String','');
set(hann,'Color','k');
hann=annotation('textarrow',[0.28 0.28],[0.874 0.627],'String','');
set(hann,'Color','k');

htext=text(0.025,1,'Depth','FontSize',14);
htext=text(0.6,10.45,'Radius','FontSize',14);
...
x = [0 eps eps 0];
y = [0 0 eps eps];
hlg=legend([har1 har2 har3 har4],'Scalp','Skull','CSF','Brain');
set(hlg,'orientation','horizontal');
set(hlg,'box','off');
lgPos=get(hlg,'Position');
set(hlg,'Position',[lgPos(1)+0.05 lgPos(2)-0.225 lgPos(3) lgPos(4)]);
hyl=ylabel('Depth (mm)')
...
%     BRAIN=8;
    ...
    hlg=legend([hpatch1 hpatch2 hpatch3 hpatch4],{'>10 J/cm^3','>1','>0.1','>0.01'});
...
%     plot(ha(4),[3 3],[muWm-semWm muWm+semWm],'k');
    ...
    set(ha(4),'Xtick',1:3);
    set(ha(4),'XTickLabel',{'CSF','Grey','White Matter'});
