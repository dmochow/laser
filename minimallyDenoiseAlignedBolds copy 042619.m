...
for s=1:nSubjects
    ...
    for e=[2 1 3]
        ...
        % construct WM regressor from echo 2
        if e==2
        ...
        % regress out white matter signal from echo 2

        oobrainTs = regressOut(obrainTs.',U(:,1:Kwm).',1).';

        % compute psc
        bsl=mean(oobrainTs(bslInds,:));
        bslTs=repmat(bsl,[size(obrainTs,1) 1]);
        pscBrainTs=(oobrainTs-bslTs)./bslTs;
        ...
        tmp=cat(1,repmat(pscBrainTs(1,:),nPad,1),pscBrainTs);
        tmpOut=filter(bf,af,tmp,[],1);
        opscBrainTs=tmpOut(nPad+1:end,:);
        
        outputBold=ts2vol(opscBrainTs,brainMask);
