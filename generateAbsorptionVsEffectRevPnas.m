...
for e=1:3  % echo index
    ...
    plot(rocx,rocy,'LineWidth',1.5);
    plot([0 1],[0 1],'--k');
    htxt(e)=text(0.05,0.8,sprintf(['AUC=%0.2f'],auc));
    axis square
...
set(get(hs(4),'XLabel'),'String','FPR','FontSize',fs-4);
set(get(hs(4),'YLabel'),'String','TPR','FontSize',fs-4);
...
set(hs(4:6),'XTick',[0 1]);
set(hs(4:6),'YTick',[0 1]);
%set(hs(5),'YTickLabel',{'','','','',''});
%set(hs(6),'YTickLabel',{'','','','',''});
...
set(hlg,'Position',[0.08 0.795 lgPos(3) lgPos(4)]);
...
for s=4:6
    subPos=get(hs(s),'Position');
    set(hs(s),'Position',[subPos(1) subPos(2) shr*subPos(3) shr*subPos(4)]);
    %set(hs(s),'Position',[0.28+(s-4)*0.24 0.85 shr*subPos(3) shr*subPos(4)]);
end
...
for s=1:3
    subPos=get(hs(s),'Position');
    set(hs(s),'Position',[xl(s) subPos(2) subPos(3) subPos(4)]);
end
yl=0.84; 
for s=1:3
    subPos=get(hs(s+3),'Position');
    set(hs(s+3),'Position',[xl(s)+0.175 yl subPos(3) subPos(4)]);
end

%%
hx=get(get(hs(4),'XLabel'),'Position');
set(get(hs(4),'XLabel'),'Position',[0.5 0 hx(3)]);

hy=get(get(hs(4),'YLabel'),'Position');
set(get(hs(4),'YLabel'),'Position',[0.1 hy(2) hy(3)]);

%%
%sublabel([hs(1) hs(2) hs(3)],-15,-30,'FontSize',16,'FontWeight','bold');
