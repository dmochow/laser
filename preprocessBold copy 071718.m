% 07/17/18
% added anatWithSkull due to weird error appearing after rerunning a
% subject
% fixed bug in generation of contralateral roi (lrfc variable)
%
% TODO: check marker coordinates and ensure no overlap between roi and
% contralateral roi
% TODO: parameterize roi mask generation with extend and add appropriate suffixes to filenames
...
for s=1:nSubjects
    ...
    anatWithSkullFilename=['../data/' subjStr '/NII/anatWithSkull+orig'];
    ...
    %% make the ROI and control ROI (in native space)
    ...
    %% registration of bold to anatomy in talairach space
    if ~ONLY_PROCESS_ROI
        cd(pathToData);
        ...
        % align the brain mask (derived from skull stripped anatomical) to
        % the talairach atlas
    ...
    %% the code below is a subset of the if-block above
