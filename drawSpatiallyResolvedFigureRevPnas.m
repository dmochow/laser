...
for e=1:3
%     isSigBefDurFilename=fullfile(pathToData,['resampled_thresh_isSigBefDur_NLAGS5_CONSTANT1_s8muwBoldEcho' num2str(e) '-Filter1-Derivative1-WhiteMatter3-STANDARDIZE1-TSHIFT1-CSHIFT2+tlrc.BRIK']);
%     isSigBefAftFilename=fullfile(pathToData,['resampled_thresh_isSigBefAft_NLAGS5_CONSTANT1_s8muwBoldEcho' num2str(e) '-Filter1-Derivative1-WhiteMatter3-STANDARDIZE1-TSHIFT1-CSHIFT2+tlrc.BRIK']);
%     
...
%% 12.05.18 pnas shrinkage
delLilUp=0.025;
delBigUp=0.045;
for z=1:10
    pos=get(hs(2,z),'Position');
    set(hs(2,z),'Position',[pos(1) pos(2)+delLilUp pos(3) pos(4)]);
    
    pos=get(hs(3,z),'Position');
    set(hs(3,z),'Position',[pos(1) pos(2)+2*delLilUp pos(3) pos(4)]);
    
    pos=get(hs(4,z),'Position');
    set(hs(4,z),'Position',[pos(1) pos(2)+2*delBigUp pos(3) pos(4)]);
end

pos=get(htim(2),'Position');
set(htim(2),'Position',[pos(1) pos(2)+2*delBigUp pos(3) pos(4)]);
