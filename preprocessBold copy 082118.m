...
% radMask=10.6; depthMask=12.5; % 95 percent min area
...
for s=1:nSubjects
    ...
    % reflect the laser to get the contralateral laser markers
    urfc=urf; ulfc=ulf; urbc=urb; lrfc=lrf;
    urfc(1)=2*midsag-urf(1);
    ulfc(1)=2*midsag-ulf(1);
    urbc(1)=2*midsag-urb(1);
    lrfc(1)=2*midsag-lrf(1);
    controlRoiMask = projectLaser(brainMaskFilename,controlLaserOrigin,urfc,ulfc,urbc,lrfc,radMask,depthMask); % project the control laser
    WriteBrikWrap(pathToData,controlRoiMask,Info,controlRoiPrefix,'orig');
    ...
    str11b=['!@auto_tlrc -apar anat+tlrc -input ' controlRoiPrefix '+orig']; eval(str11b);
    str11c=['!gunzip ' controlRoiPrefix '+tlrc.BRIK.gz']; eval(str11c);
    str13=['!3dresample -master dsbold_e2_tlrc_al+tlrc -prefix resampled_' roiPrefix ' -input ' roiPrefix '+tlrc']; eval(str13);  % resample roi on bold grid
    str13b=['!3dresample -master dsbold_e2_tlrc_al+tlrc -prefix resampled_' controlRoiPrefix ' -input ' controlRoiPrefix '+tlrc']; eval(str13b);
    cd(origPath);
