...
for s=1:nSubjects
    ...
    switch subjStr
        ...
        case 'S11'
            ...
            midsag=111;
            ulfc=[2*midsag-127 255-48 197]+[1 1 1];
            urfc=[2*midsag-148 255-55 193]+[1 1 1];
            urbc=[2*midsag-161 255-25 213]+[1 1 1];
            lrfc=[2*midsag-149 255-30 158]+[1 1 1];
    ...
    if exist('ulf') && exist('urf') && exist ('urb') && exist ('lrf')
        ...
        controLaserOrigin = getLaserOrigin(ulfc,urfc,urbc);
        
        % TODO: put the following block into a function
        % fineTuneOrigin
        ...
        save(fullfile(pathToData,'laserOrigin.mat'),'laserOrigin','findind','imageInt');
        
        % the control laser
        
        crossprod=cross((ulfc-urfc),(ulfc-lrfc));
        crossprod=crossprod/norm(crossprod);
        if crossprod(2)>0 % if pointing anterior
            crossprod=-crossprod;
        end
        xo=controlLaserOrigin;
        imageInt=zeros(MAXITER,1);
        for i=1:MAXITER
            xo=xo+crossprod*STEPSIZE;
            rxo=round(xo);
        ...
        findind=find(dImageInt>SCALP_INT_THRESH,1);
        controlLaserOrigin=controlLaserOrigin+findind*STEPSIZE*crossprod;
        
        save(fullfile(pathToData,'controlLaserOrigin.mat'),'controlLaserOrigin','findind','imageInt');
